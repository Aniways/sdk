...
### 2. Add The Aniways project your work set
 ...
 2. Import this project into your eclipse workspace (File -> Import -> General -> Existing projects into workspace -> Browse). Sometimes a "Clean" is needed here.
 3. Add it as an Android Library Project used by your app (Right-click your Android App Project -> Properties -> Android -> under "Library" click Add... -> Select Aniways -> OK -> OK).
...
This initialization also starts the Aniways service which syncs the phrases and emoticons definitions with the Aniways server.
...
private void sendMessage() {
       ...
       String text = Aniways.replaceAniwaysIconsWithText(editableText);
       editView.setText("");
       sendMessage(text);
...
Don't forget to import `com.aniways.Aniways`
...
In order to activate Aniways you need to set the following mandatory configuration parameters:
    ...
    <resources>
	    ...
	    <!--
	       ...
	       Your App's upgrade URL. Needs to be a url which will redirect to the 
...
All of these properties are documented in the 'Guids' section on the site, or in the 'Aniways Configuration.docx' file which in the SDK zip file. 

Add permissions in the AndroidManifest.xml file
Add the following permissions to your AndroidManifest.xml (directly under <manifest>):

(No need to add permissions that already exist)
...
### 9. Add the Aniways services to the AndroidManifest
...
### 10. Test the basic Aniways experiance in your App
...
### 11. Add the Aniways Emoticons on Demand Button
...
There are 3 steps to configure the button: (There is lots of text here, but when you look at the code samples in the middle and end you see how simple it is smiley )

Place an ImageView, or an ImageButton where you want the button to be placed in your layout. You can set its size, background color, etc. (to configure the displayed images, you will need to change the Aniways resources).
Include the aniways_emoticons_button_popup_placeholder layout at the bottom most part of the vertical Linear Layout which contains the layout of the activity in which the AniwaysEditText is placed. Set the width to ‘match_parent’ and hight to ‘wrap_content’. This layout has a visibility of ‘Gone’ by default. The idea is that it will be positioned where the keyboard would be when it is opened, and so if the keyboard is not opened, and the button is pressed then Aniways would change its visibility to ‘Visible’ and make it push the AniwaysEditText up, like the keyboard would do, and create room for the content of the button to be displayed.
...
<activity android:name="com.aniways.billing.AniwaysCreditsStoreActivity" android:theme="@style/Theme.Aniways.Transparent" android:launchMode="singleTop" />
Please note that more info on the credits store can be found in the “Aniways Credits Store Guide.docx” file in the SDK zip.

 

Additional info
You can find additional documentation about our SDK on this site and in the SDK zip file you have downloaded.

API documentation - ‘Aniways javadocs.zip’ .

Configuration - “Aniways Configuration.docx” .

Emoticons button - “Aniways Emoticons Button Guide.docx” .

Credits store - “Aniways Credits Store Guide.docx” .

 

Contact info
You are also welcomed to contact us via Skype or Mail for any question or problem:

Shai Ber (CTO) – Mail: shai@aniways.com , Skype: shaiber01, Phone: 972-54-4220356

Ram Greenberg (CPO) – Mail: ram@aniways.com , Skype: ramgreenberg977, Phone: 972-52-3467582
